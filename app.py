...
from modules.mask_utils import save_selected_mask
                ...
                with gr.Tabs():
                    ...
                    with gr.TabItem(_("Layer Divider")):
                        ...
                        btn_generate.click(fn=self.sam_inf.divide_layer,
                                           inputs=input_params, outputs=[gallery_output, output_file, masks_state])
                        # 新增：点击gallery中某一张mask，并保存该mask为二值图
                        gallery_output.select(
                            fn = save_selected_mask,
                            inputs=[img_input, masks_state],
                            outputs=None
                        )
