...
class SamInference:
    ...
    def divide_layer(self,
        ...
        if input_mode == AUTOMATIC_MODE:
            ...
            generated_masks = self.format_to_auto_result(predicted_masks, scores)
        ...
        return gallery, output_path, generated_masks
    ...
    def format_to_auto_result(
        ...
        result = []
        for i, mask_group in enumerate(masks):  # mask_group: (K, H, W)
            for j, mask in enumerate(mask_group):
                result.append({
                    "segmentation": mask,
                    "score": float(scores[i][j]),  # score may be numpy.float32, so convert to float,
                    "area": place_holder
                })
        return result
